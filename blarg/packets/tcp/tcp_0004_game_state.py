...
from blarg.constants.constants import SKIN_MAP, TEAM_MAP
import os
# class tcp_0004_game_state:
#     def __init__(self):
#         pass

#     def serialize(self, data: deque):
#         print("".join(list(data)))
#         for i in range(36): data.popleft()
#         names = []
#         while len(data) > 0 and  data[0] != "FF" :
#             current = ''
#             while data[0] != '00':
...
#             data.popleft()
#         if len(data) == 0:
#             return {'event':9, 'names':[]}

#         # if len(names) > 1:
#         #     names.pop()
#         names = [str(bytes(bytearray.fromhex(name)), 'utf-8') for name in names]

        
#         result = {}
#         code = data.pop()
#         if code == '00':
#             result['event'] = 9 #game starting
#         else:
#             if len(names) <= 1:
#                 result['event'] = 10 #game createdd
#             else:
#                 result['event'] = 11 #player joined
#         result['names'] = names
